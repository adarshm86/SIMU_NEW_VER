...
const CLASS_COLORS = {
  ...
  4: new THREE.Color("#334155"), // Dark Gray -> Dead Cell
};
...
export default function PetriDish3D({
  ...
  useEffect(() => {
    ...
    const shadowBase = new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({ color: 0x01050e, transparent: true, opacity: 0.88 })
    );
    ...
    const culturePlate = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: 0x061124, metalness: 0.1, roughness: 0.75 })
    );
    ...
    const heatPlane = new THREE.Mesh(
      new THREE.PlaneGeometry(11.0, 11.0),
      new THREE.MeshBasicMaterial({ map: heatTexture, transparent: true, opacity: 0.85, side: THREE.DoubleSide, blending: THREE.AdditiveBlending })
    );
    ...
    // LIGHTING
    scene.add(new THREE.HemisphereLight(0xffffff, 0x07111f, 1.2));
    scene.add(new THREE.AmbientLight(0xffffff, 0.8));
    const keyLight = new THREE.DirectionalLight(0xffffff, 2.0);
    keyLight.position.set(8, 12, 7);
    scene.add(keyLight);

    // BACTERIA GEOMETRY - Perfect spheres sized exactly to the grid cell to prevent overlap
    // 11.0 grid width / 40 cells = ~0.27 units per cell. Radius of 0.12 = 0.24 diameter. Perfect fit!
    const bacteriaGeo = new THREE.SphereGeometry(0.12, 24, 24);

    // UPDATED MATERIAL: Color is white so instance colors apply correctly.
    // OLD CODE - INCORRECT
    // NEW CODE - CORRECTED
    const bacteriaMat = new THREE.MeshStandardMaterial({
      color: 0xffffff, // Acts as a pure canvas for your instance colors
      roughness: 0.2,
      metalness: 0.1,
      transparent: true,
      opacity: 0.95,
    ...
    const animate = () => {
      ...
      for (let i = 0; i < visibleCount; i++) {
        ...
        // PERFECT GRID ALIGNMENT: 1 Cell = 1 Grid Block. No overlap jitter.
        const xNorm = (cell.x / Math.max(1, gridSize - 1)) * 2 - 1;
        const yNorm = (cell.y / Math.max(1, gridSize - 1)) * 2 - 1;
        const px = xNorm * 5.5; 
        const pz = yNorm * 5.5;

        // Subtle biological pulsing, scaling equally so it stays a sphere
        const pulse = isCellAlive ? (1.0 + Math.sin(time * 4.0 + i * 0.42) * 0.1) : 0.8;
        const scale = pulse;
        const lift = isCellAlive ? 0.05 : 0.0;

        dummy.position.set(px, lift, pz);
        dummy.scale.set(scale, scale, scale);
        // Removed rotation so spheres don't distort
        dummy.updateMatrix();
        instancedMesh.setMatrixAt(i, dummy.matrix);

        // PERFECT COLORS mapped directly from CLASS_COLORS
        if (isCellAlive) {
            tempColor.copy(CLASS_COLORS[cell.cls] ?? CLASS_COLORS[0]);
        } else {
            tempColor.copy(CLASS_COLORS[4]); // Dark Gray Dead
        }
    ...
    sceneRef.current = {
      scene, camera, renderer, controls, instancedMesh, dummy, heatCanvas, heatTexture, mount, tempColor
    };
  }, [gridSize]);
      ...
      for (let y = 0; y < size; y++) {
        for (let x = 0; x < size; x++) {
          ...
          if (warmth < 0.15) {
            ...
            imgData.data[idx + 1] = Math.round(77 + warmth * 150);
            imgData.data[idx + 2] = 255;
    ...
    <div className="relative w-full h-full min-h-[500px] flex flex-col justify-between">
      ...
      <div className="pointer-events-none absolute top-16 right-3 rounded-xl border border-cyan-400/30 bg-navy-deep/85 px-3.5 py-2.5 text-[10px] font-data text-medical-white/90 backdrop-blur-md shadow-glass">
        ...
        <div className="space-y-1.5">
          <div className="flex items-center gap-2"><span className="h-2.5 w-2.5 rounded-full bg-[#34D399] shadow-[0_0_8px_#34D399]" />Susceptible</div>
          ...
          <div className="flex items-center gap-2"><span className="h-2.5 w-2.5 rounded-full bg-[#334155] opacity-60" />Dead Cell</div>
        </div>
      </div>
